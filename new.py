...
def build_validation_prompt(user_question: str, generated_sql: str, schema_context: dict) -> str:
    """
    Create a detailed validation prompt for the LLM to check if the SQL query is valid.
    It will also check if the query truly aligns with the user's intent.
    """
    schema_info = "\n".join([f"Table: {table['name']}, Columns: {', '.join(table['columns'])}" for table in schema_context['tables']])

    validation_prompt = f"""
    You are an expert SQL assistant. Your task is to validate the following SQL query in the context of the user's question and the schema provided.
    ...
    ### Task:
    1. **Does the SQL query match the user's intent** based on the question?
       - Does the query address all aspects of the user's question?
       - Are all key concepts in the question (like aggregates, filters, or groupings) captured in the SQL query?

    2. **Are all necessary tables and columns correctly used**?
       - Are the appropriate tables and columns referenced in the SQL query? If any required ones are missing, please point them out.

    3. **Is the SQL query syntactically correct and logically sound**?
       - Does the SQL query execute without errors based on the schema provided?
       - Are there any potential issues like missing JOINs or logical contradictions?

    4. **Does the query appear safe**?
       - Look out for any potential security issues like SQL injection, incorrect joins, or other bad practices.

    ### Response Format:
    {{
        "valid": <true/false>,
        "explanation": "<Explanation of why the query is valid or invalid.>"
    }}
    """
    return validation_prompt


def validate_and_provide_feedback(user_question: str, generated_sql: str, schema_context: dict) -> dict:
    """
    Validate the SQL query and provide feedback or fallback.
    Includes suggestions for improvement based on user query and schema.
    """
    validation_prompt = build_validation_prompt(user_question, generated_sql, schema_context)

    # Call the LLM API for validation
    validation_response = validate_sql_with_llm(validation_prompt)

    # Process the validation response
    if validation_response.get("valid", False):
        return {
            "sql": generated_sql,
            "clarification": "",
            "explanation": "Query is valid and matches the user's intent."
        }
    ...
    fallback_response = handle_invalid_query(user_question, generated_sql, explanation, schema_context)

    return fallback_response

def handle_invalid_query(user_question: str, generated_sql: str, explanation: str, schema_context: dict) -> dict:
    """
    Handle the case when the generated SQL is invalid, and fallback mechanisms are triggered.
    Provides more specific and actionable feedback based on schema context.
    """
    # Fallback Option 1: Suggest a better SQL query using the schema context
    fallback_sql = suggest_query_modification(user_question, generated_sql, schema_context)

    if fallback_sql:
        return {
            "sql": fallback_sql,
            "clarification": "Query was adjusted based on the validation feedback.",
            "explanation": explanation + " Adjusted SQL query."
        }

    # Fallback Option 2: Ask for specific clarification from the user
    clarification = get_specific_clarification(user_question, explanation)

    return {
        "sql": "",
        "clarification": clarification,
        "explanation": explanation + " More information is needed to clarify the query."
    }

def suggest_query_modification(user_question: str, generated_sql: str, schema_context: dict) -> str:
    """
    Suggest modifications to the SQL query based on validation feedback and schema context.
    Tries to make intelligent suggestions like adding WHERE conditions or changing SELECT columns.
    """
    # Example modification logic based on schema context
    if "SELECT *" in generated_sql:
        # Suggest selecting specific columns based on the schema
        return generated_sql.replace("SELECT *", f"SELECT {', '.join(schema_context['tables'][0]['columns'])}")  # Just an example

    # Example of adding a WHERE condition if it's missing
    if "WHERE" not in generated_sql:
        return f"{generated_sql} WHERE some_column > 100"  # Example adjustment

    return None  # Return None if no modification is possible automatically

def get_specific_clarification(user_question: str, explanation: str) -> str:
    """
    Generate a specific clarification question for the user based on the explanation.
    This helps narrow down the ambiguity in the user query.
    """
    if "high notional" in user_question:
        return "What threshold should define 'high notional'? Please provide a numeric value."

    if "group by" in explanation:
        return "Which column should the results be grouped by? Please specify."

    return "Could you clarify what exactly you want to see in the results? It seems there might be some ambiguity."

def clarify_user_query(user_question: str) -> str:
    """
    Use a model to help clarify ambiguous or unclear user queries.
    This can be done by prompting the model to provide a clearer version of the question.
    """
    clarification_prompt = f"""
    You are an expert SQL assistant. The user provided the following question, but it might be unclear:

    ### User Question:
    {user_question}

    Your task is to rephrase or clarify this question in a more specific way that would help in generating a more accurate SQL query.
    ...
    response = requests.post(
        ...
        headers={"Content-Type": "application/json", "x-api-key": llama_3_key}
    )

    if response.status_code == 200:
        return response.json()["choices"][0]["message"]["content"]

    return user_question  # Return the original query if something goes wrong
...
# Assuming necessary functions like generate_sql, execute_sql, create_interactive_visualization, display_data_summary, etc. are defined elsewhere.
...
def main():
    ...
    if "clarification_attempts" not in st.session_state:
        st.session_state.clarification_attempts = 0  # Track how many clarification attempts have been made
    ...
    if st.session_state.conversation_state == "asking":
        ...
        # Clarification section with an animation class
        st.markdown('<div class="clarification-section slide-up">', unsafe_allow_html=True)

        # Display the need for clarification
        ...
        # Input field for user to provide more clarification
        ...
        with col1:
            # Handle the "Submit Clarification" button click
            if st.button("✅ Submit Clarification", type="primary"):
                if clarification.strip():  # Check if the user entered clarification
                    # Combine user question and clarification
                    full_question = f"{st.session_state.user_question}. Clarification: {clarification}"

                    # Process the clarified question
                    with st.spinner("🔄 Processing your clarified question..."):
                        result = generate_sql(full_question)

                    # Increment the clarification attempt counter
                    st.session_state.clarification_attempts += 1

                    # Check if we need to keep clarifying or move forward
                    if st.session_state.clarification_attempts >= 5:
                        st.session_state.conversation_state = "asking"
                        st.session_state.clarification_question = ""
                        st.warning("⚠️ Too many ambiguous clarifications. Please restart your question with more details.")
                        st.rerun()  # Restart the process if too many attempts
                    else:
                        # Update session state and move to the results state
                        st.session_state.sql_result = result
                        st.session_state.final_question = full_question
                        st.session_state.conversation_state = "results"
                        st.rerun()
                else:
                    # If clarification is empty, prompt user to provide clarification
                    st.warning("⚠️ Please provide clarification")

        with col2:
            # Handle the "Start Over" button click
            if st.button("🔄 Start Over"):
                # Reset the conversation state and related session variables
                st.session_state.conversation_state = "asking"
                st.session_state.user_question = ""
                st.session_state.clarification_question = ""
                st.session_state.clarification_attempts = 0  # Reset attempts
                st.rerun()  # Restart the process

        # Close the clarification section with the animation
        st.markdown('</div>', unsafe_allow_html=True)
